...
*Your reputation is your most valuable asset*
    ...
    async def show_top_sellers(self, query, user_id):
        """Show top sellers with verified status"""
        with self.flask_app.app_context():
            # Get users who have completed 5+ deals with proper context handling
            top_sellers_data = db.session.query(
                User.id, User.first_name, db.func.count(Deal.id).label('deal_count')
            ).join(Deal, 
...
*Start dealing and build your reputation!*
                """
            else:
                text = "⭐ **Top Verified Sellers**\n\n"
                text += "🏆 *These sellers have completed 5+ deals and earned their verified status*\n\n"
                
                for i, (seller_id, seller_name, completed_deals) in enumerate(top_sellers_data, 1):
                    # Calculate rating based on deal count
                    rating = min(5.0, 4.0 + (completed_deals * 0.1))
                    stars = "⭐" * int(rating)
                    
                    text += f"""
**#{i}** 🏅 **{seller_name}**
{stars} **{rating:.1f}** • {completed_deals} deals completed
💼 *Verified Seller*
━━━━━━━━━━━━━━━━━━━━━

"""
                
                text += "\n💡 *Want to become a Top Seller? Complete more deals and build your reputation!*"
